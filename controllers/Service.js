...
class Service {
    ...
    async getAll() {
        const services = await this.Model.find({}).populate({
            path: 'createdBy',
    ...
    async createService(request, response) {
        ...
        const serviceDetails = await this.Model.findByIdAndUpdate(data._id, { $set: data }, {
            upsert: true, new: true,
            setDefaultsOnInsert: true, runValidators: true
        }).then(result => {
            return response.status(200).send({ message: "Service " + (newService ? "Added" : "Updated") + " Successfully." })
        }).catch(e => {
            return e
        });
